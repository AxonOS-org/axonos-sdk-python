...
### Added
- `examples/demo.py` — a runnable, dependency-free end-to-end demo (synthetic
  session) that builds real `IntentObservation` records, proves the RFC-0006
  32-byte wire round-trip byte-for-byte, and runs the capability gate
  (including structural rejection of raw-signal access). Doubles as living
  documentation. CI now also lints `examples/`.
